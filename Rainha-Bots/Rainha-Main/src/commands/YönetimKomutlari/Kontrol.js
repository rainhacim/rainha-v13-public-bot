...
module.exports = {
    ...
    return }
       ...
       let guild = client.guilds.cache.get(god.GuildID);
...
.setThumbnail(message.author.displayAvatarURL({ dynamic: true, size: 2048 }))
  ...
  let msg = await message.channel.send({ embeds: [rainha], components: [row]})
    ...
    let rainha = message.guild.members.cache.filter(member => !member.roles.cache.has(etkinlik) || !member.roles.cache.has(cekilis))
    button.reply({ content:`
Etkinlik/Çekiliş rolü olmayan ${rainha.size} kullanıcıya etkinlik, çekiliş rolleri verildi !`})
...
${taglilar.map(x => x || "Rolü olmayan Kullanıcı bulunmamaktadır.")}`})
    ...
    button.reply({ content:`
Kayıtsız rolü olmayan ${god.size} kullanıcıya kayıtsız rolü verildi !

Kayıtsız Rolü verilen kullanıcılar;
${god.map(x => x || "Rolü olmayan Kullanıcı bulunmamaktadır.")} `})
