...
module.exports = {
  ...
  run: async (client, message, args) => {  
    ...
    if (args[0] === "oluştur" || args[0] === "ekle") {
        ...
        message.channel.send({ embeds: [new MessageEmbed().setDescription(`${green} \`${komutAd}\` adlı komut başarılı bir şekilde oluşturuldu.\n\nKomut İzni Olan Rol(ler): ${yetkiliRol}\nVerilecek Rol(ler): ${roller}\nKomut Log Kanal: ${kanalcık}`)] });
        ...
        if (!data2) return message.reply({ embeds: [new MessageEmbed().setDescription(`${red}  Lütfen bir komut adı girerek tekrar deneyiniz.\n\n(Komutlar: \`${data.map(x => x.komutAd).join(" - ")}\`) `)] }).then((e) => setTimeout(() => { e.delete(); }, 5000)).catch(err => {});
        message.channel.send({ embeds: [new MessageEmbed().setDescription(`Komut adı : ${data2.komutAd}\nRol : ${data2.verilecekRol.length > 0 ? data2.verilecekRol.map(x => `<@&${x}>`) : "Her hangi bir rol yok."}\nYetkililer : ${data2.YetkiliRol.length > 0 ? data2.YetkiliRol.map(x => `<@&${x}>`) : "Her hangi bir rol yok."}\nKomut Log Kanal: ${data2.kullanımKanal ? data2.kullanımKanal : `Her hangi bir kanal yok.`} `)] })
