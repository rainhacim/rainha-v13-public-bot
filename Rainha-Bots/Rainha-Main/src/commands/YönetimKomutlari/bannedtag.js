...
module.exports = {
  ...
  run: async (client, message, args, perm, prefix) => {
        ...
        const data = await bannedTag.findOne({ guildID: god.GuildID });
            ...
            if (args[0] == "ekle") {
                ...
                if (!data) {
                   ...
                   await bannedTag.findOneAndUpdate({ guildID: message.guild.id }, { $push: { tags: args[1] } }, { upsert: true });
                    üyeler.map(x => {
                        if (x.roles.cache.has(conf.yasaklıtagRole)) return
                        setTimeout(() => {
                            x.setNickname('Yasaklı Tag');
                            x.roles.cache.has(conf.boosterRolu) ? x.roles.set([conf.boosterRolu, conf.yasaklıtagRole[0]]) : x.roles.set(conf.yasaklıtagRole)
                        }, 1000)
                    })
                } else {
            ...
            if (args[0] == "liste" && !args[1]) {
                ...
                let num = 1
                let arrs = data.taglar.map(x => `\`${num++}.\` ${x} - (${client.users.cache.filter(s => data.taglar.some(rainha => s.tag.includes(rainha))).size} üye)`)
            ...
            if (args[0] == "kontrol") {
                ...
                data.taglar.forEach(x => {
                    let üye = message.guild.members.cache.filter(mems => {
                        return mems.user.tag.includes(x) && !mems.roles.cache.has(conf.yasaklıtagRole)
                    }).map(x => x.id)
                    message.channel.send({ content:`${x} tagı bulunup <@&${conf.yasaklıtagRole}> rolü olmayan ${üye.length} kişiye rolü veriyorum.`})
                    for (let i = 0; i < üye.length;i++) {
                        setTimeout(() => {
                            message.guild.members.cache.get(üye[i]).roles.cache.has(conf.boosterRolu) ? message.guild.members.cache.get(üye[i]).roles.set([conf.boosterRolu, conf.yasaklıtagRole[0]]) : message.guild.members.cache.get(üye[i]).roles.set(conf.yasaklıtagRole)
                        }, (i + 1) * 1000)
                    }
                })
            }
