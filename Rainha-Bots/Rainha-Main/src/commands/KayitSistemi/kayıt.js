...
run: async (client, message, args, embed, prefix) => { 
    ...
    {
    message.react(red)
    message.reply({ content:`\`${prefix}kayıt <@rainha/ID> <Isim> <Yaş>\``}).then((e) => setTimeout(() => { e.delete(); }, 5000)); 
    return }
   ...
   const tagModedata = await regstats.findOne({ guildID: message.guild.id })
    ...
    const row = new MessageActionRow()
		.addComponents(
    new MessageButton().setCustomId("MAN").setLabel("Erkek").setStyle("PRIMARY").setEmoji("1043180551231770654"),
    new MessageButton().setCustomId("WOMAN").setLabel("Kadın").setStyle("SUCCESS").setEmoji("1114846132502265857"),
    new MessageButton().setCustomId("İPTAL").setLabel("İptal").setStyle("DANGER").setEmoji("1083791139162832956"),
	);
...
if(button.customId === "MAN") {
  ...
  if(msg) msg.delete();
  button.reply({ embeds: [rainhae], components: [], ephemeral: true});

    await uye.roles.add(ayar.erkekRolleri)
    await uye.roles.remove(ayar.unregRoles)
    await coin.findOneAndUpdate({ guildID: uye.guild.id, userID: message.author.id }, { $inc: { coin: god.Main.toplamsCoin } }, { upsert: true });
...
if(msg) msg.delete();
button.reply({ embeds: [rainhak], components: [], ephemeral: true});
    ...
    if (uye && uye.voice && uye.voice.channel && ayar.registerParents.includes(uye.voice.channel.parentId)) {
      ...
      .setAuthor({ name: uye.user.tag, iconURL: uye.displayAvatarURL({ dynamic: true }) })
      .setColor('2f3136')
      .setDescription(`${uye} kullanıcısı ${message.member} tarafından **Kadın** olarak kayıt edildi.`)
      .addFields([
        { name: 'Kayıt Edilen Kullanıcı', value: `${uye.toString()}`, inline: true },
        { name: 'Kayıt Eden Kullanıcı', value: `${message.member.toString()}`, inline: true },
        { name: 'Kayıt Tarihi', value: `<t:${Math.floor(Date.now() / 1000)}:R>` }
