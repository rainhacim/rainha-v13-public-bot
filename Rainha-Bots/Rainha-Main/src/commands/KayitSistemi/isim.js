...
module.exports = {
  ...
  run: async (client, message, args, perm, prefix) => {
    ...
    {
    message.react(red)
    message.reply({ content:`\`${prefix}isim <@rainha/ID> <Isim> <Yaş>\``}).then((e) => setTimeout(() => { e.delete(); }, 5000)); 
    return }
...
.setFooter({ text: `Üyesinin önceki isimlerine <!isimler @rainha/ID> komutuyla bakarak kayıt işlemini gerçekleştirmeniz önerilir.` });
