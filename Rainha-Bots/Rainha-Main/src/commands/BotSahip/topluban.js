...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    const god = await message.reply({ content: 
        `${members
            .map((member, idx) => `**${idx + 1}. ${member.toString()}**`)
            .join("\n")}\nBu üyeleri banlamak istiyor musun?` })

    await god.react(green);

    const filter = (reaction, user) => {
        return reaction.emoji.name === 'green' && user.id === message.author.id;
    };
    const collector = god.createReactionCollector({ filter, time: 10000 });

    collector.on("collect", async () => {
        await god.edit({ content: `${green} ${members.length} adet kullanıcı başarıyla yasaklandı.`})
    ...
    collector.on("end", (_, reason) => {
        if (reason === "time")
            god.edit({ content: `10 saniye geçtiği için işlem iptal edildi.`})
    });
