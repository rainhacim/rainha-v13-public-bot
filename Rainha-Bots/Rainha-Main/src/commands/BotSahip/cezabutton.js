...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    client.api.channels(message.channel.id).messages.post({
      data: {
        "content": `${Jail} **Aşağıda Bulunan Butonlardan Ceza Listenizi, Ceza Puanınızı Ve Aktif Cezanızın Kalan Süresini Görüntüleyebilirsiniz.**`, "components": [{
          "type": 1, "components": [
...
client.on('interactionCreate', async interaction => {
  ...
  const cezaData = await ceza.findOne({ guildID: god.GuildID, userID: member.id });
  const cezapuanData = await cezapuan.findOne({ guildID: god.GuildID, userID: member.id });
 ...
 Toplamda \`${cezapuanData ? cezapuanData.cezapuan : 0} ceza puanı\` ve (Toplam **${cezaData ? cezaData.ceza.length : 0}** Ceza) olarak gözükmekte.`, ephemeral: true
  ...
  let data = await penals.find({ guildID: god.GuildID, userID: member.id, active: true }).sort({ date: -1 });
  ...
  let datas = await penals.find({ guildID: god.GuildID, userID: member.id, active: true }).sort({ date: -1 });
  ...
  if (interaction.customId === "kalanzaman") {
    if (data.length === 0) return interaction.reply({ content: `${member.toString()} **üyesinin aktif ceza bilgisi bulunmamaktadır.**`, ephemeral: true})
