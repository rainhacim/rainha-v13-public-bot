...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    await penals.find({ guildID: message.guild.id, userID: member.user.id, }).sort({ date: -1 }).exec(async (err, res) => {
        ...
        let rainha = table(xd.slice(0, 15), config)
        let god = table(xd2, config)
        ...
        .addComponents(
          ...
          new MessageButton().setCustomId('iptal').setEmoji("❌").setStyle('DANGER'),
        );
    ...
    if (button.customId === "dosya") {
      ...
      button.reply({ content: `${member} kullanıcısının toplamda **${cezaData ? cezaData.ceza.length : 0}** cezası  bulunmaktadır ve aşağıdaki belgede yazmaktadır.`, files: [{ attachment: Buffer.from(rainha), name: `${member.user.username}_sicil.txt` }], components: [], ephemeral: true }); 
