...
module.exports = {
  conf: {
    aliases: ["jail","temp-jail","ceza"],
    name: "jail",
    help: "jail <@isim/ID> <Süre> <Sebep>",
  ...
  run: async (client, message, args, embed) => {
    ...
    if (god.Main.jaillimit > 0 && jailLimit.has(message.author.id) && jailLimit.get(message.author.id) == god.Main.jaillimit) 
    {
    message.react(red)
...
if (duration) {
  ...
  if (god.Main.dmMessages) member.send({ content:`**${message.guild.name}** sunucusunda, **${message.author.tag}** tarafından, **${reason}** sebebiyle, <t:${Math.floor((Date.now() + duration) / 1000)}:R>'ya kadar jaillendiniz.`}).catch(() => {});
