...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    if (god.Main.banlimit > 0 && banLimit.has(message.author.id) && banLimit.get(message.author.id) == god.Main.banlimit) return message.channel.send({ content:"Saatlik ban sınırına ulaştın!"}).then((e) => setTimeout(() => { e.delete(); }, 5000));
    message.react(green)
    if (god.Main.dmMessages) user.send({ content:`**${message.guild.name}** sunucusundan, **${message.author.tag}** tarafından, **${reason}** sebebiyle banlandınız!`}).catch(() => {});
    ...
    .setImage("https://cdn.discordapp.com/attachments/1061350563775783004/1064216448706281632/a_4157779f8043232e0ed42f9dc918cfa7.gif")
