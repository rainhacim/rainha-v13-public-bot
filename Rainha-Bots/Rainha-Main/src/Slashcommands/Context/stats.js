const { MessageEmbed, MessageActionRow, MessageSelectMenu, IntegrationApplication, MessageAttachment } = require("discord.js");
...
module.exports = {
    ...
    async execute(interaction, client) {
        ...
        const toplamData = await toplams.findOne({ guildID: interaction.guild.id, userID: member.id });
    
    
    const inviterData = await inviterSchema.findOne({ guildID: interaction.guild.id, userID: member.id });
        const total = inviterData ? inviterData.total : 0;
    
            const category = async (parentsArray) => {
            const data = await voiceUserParent.find({ guildID: interaction.guild.id, userID: member.id });
    ...
    const ctx = canvas.getContext('2d');
     ...
     let rainha = Math.floor((Math.random() * god.length));
      ...
      const yazı = [] 
      if(member.username.length > 8) {
        let yarrak = member.username.slice(0, 8)
         yazı.push(`${yarrak}...`)  
            } else {
        yazı.push(`${member.username}`)
                }
     ...
     ctx.save();
      ...
      const attachment = new MessageAttachment(canvas.toBuffer(), 'rainha.png');
      interaction.reply({ content:`> **[** \`${member.tag}\` **]** kullanıcısının sunucu veri kartı!`, files:  [attachment], ephemeral: true })
