...
if (interaction.member.permissions.has("ADMINISTRATOR")) {
    ...
    interaction.reply({ embeds: [new MessageEmbed().setThumbnail(interaction.user.avatarURL({ dynamic: true, size: 2048 })).setDescription(`${interaction.user}, ${member} kişisini yanınıza gittiniz.`)], ephemeral: true });
...
let msg = await interaction.reply({ content: `${member}`, embeds: [rainha], components: [row] })
