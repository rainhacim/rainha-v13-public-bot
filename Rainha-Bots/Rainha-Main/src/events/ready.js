...
client.guilds.cache.get(god.GuildID).members.fetch()

  const { joinVoiceChannel } = require("@discordjs/voice");


    const VoiceChannel = client.channels.cache.get(god.BotSesKanal);
    joinVoiceChannel({
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  const newData = new bannedTag({ guildID: god.GuildID })
...
async function TagAlıncaİsimKontrol() { // Tag alınca tarama
  const guild = client.guilds.cache.get(god.GuildID)
...
const guild = client.guilds.cache.get(god.GuildID);
let god = guild.members.cache.filter(m => m.roles.cache.filter(r => r.id !== guild.id).size == 0)
   god.forEach(r => {
