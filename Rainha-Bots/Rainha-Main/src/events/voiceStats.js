...
module.exports = async (oldState, newState) => {
  ...
  async function saveData(user, channel, data) {
    await voiceUser.findOneAndUpdate({ guildID: god.GuildID, userID: user.id }, { $inc: { topStat: data, dailyStat: data, weeklyStat: data, twoWeeklyStat: data } }, { upsert: true });
    await voiceGuild.findOneAndUpdate({ guildID: god.GuildID }, { $inc: { topStat: data, dailyStat: data, weeklyStat: data, twoWeeklyStat: data } }, { upsert: true });
    await guildChannel.findOneAndUpdate({ guildID: god.GuildID, channelID: channel.id }, { $inc: { channelData: data } }, { upsert: true });
    await userChannel.findOneAndUpdate({ guildID: god.GuildID, userID: user.id, channelID: channel.id }, { $inc: { channelData: data } }, { upsert: true });
    if (channel.parent) await userParent.findOneAndUpdate({ guildID: god.GuildID, userID: user.id, parentID: channel.parentId }, { $inc: { parentData: data } }, { upsert: true });
  
    await client.checkLevel(user.id, god.GuildID, "ses")
    ...
    if (channel.parent && conf.publicParents.includes(channel.parentId)) {
        if (data >= (1000 * 60) * god.Main.voiceCount) await dolar.findOneAndUpdate({ guildID: god.GuildID, userID: user.id }, { $inc: { dolar: god.Main.voiceDolar * parseInt(data/1000/60) } }, { upsert: true });
      } else if (data >= (1000 * 60) * god.Main.voiceCount) await dolar.findOneAndUpdate({ guildID: god.GuildID, userID: user.id }, { $inc: { dolar: god.Main.voiceDolar * parseInt(data/1000/60) } }, { upsert: true });
  }

  async function saveDatas(user, channel, data) {
    if (conf.staffs.some(x => user.member.roles.cache.has(x))) {
      if (channel.parent && conf.publicParents.includes(channel.parentId)) {
        if (data >= (1000 * 60) * god.Main.voiceCount) await coin.findOneAndUpdate({ guildID: user.guild.id, userID: user.id }, { $inc: { coin: Math.floor(parseInt(data/1000/60) / god.Main.voiceCount) * god.publicCoin } }, { upsert: true });
      } else if (data >= (1000 * 60) * god.Main.voiceCount) await coin.findOneAndUpdate({ guildID: user.guild.id, userID: user.id }, { $inc: { coin: Math.floor(parseInt(data/1000/60) / god.Main.voiceCount) * god.voiceCoin } }, { upsert: true });
  ...
  await Stat.findOneAndUpdate({ guildID: god.GuildID, userID: id }, { $inc: { voiceXP: xp * randomVoiceXP } }, { upsert: true });
  const data = await Stat.findOne({ guildID: god.GuildID, userID: id });
  let siradakilevel = data.voiceLevel * 643;
  if (siradakilevel <= data.voiceXP) { 
  await Stat.findOneAndUpdate({ guildID: god.GuildID, userID: id }, { $inc: {["voiceLevel"]: 1 }}, { upsert: true });
  }
  };
