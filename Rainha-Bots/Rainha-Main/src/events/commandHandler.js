...
module.exports = async (message) => {
  let prefix = god.Main.prefix.find((x) => message.content.toLowerCase().startsWith(x));
  ...
  const embed = new MessageEmbed().setFooter({ text: god.AltBaşlık}).setColor(message.member.displayHexColor).setAuthor({ name: message.member.displayName, iconURL: message.author.avatarURL({ dynamic: true })});
  ...
  if (cmd) {
    if (cmd.conf.owner && !god.owners.includes(message.author.id)) return;
    ...
    `)
    if(komutLog) komutLog.wsend({ embeds: [rainha]})
  }
};
