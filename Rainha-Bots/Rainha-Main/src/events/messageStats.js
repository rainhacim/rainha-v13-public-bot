...
module.exports = async (message) => {
  if (message.author.bot || !message.guild || message.content.startsWith(god.Main.prefix)) return;
  
  if (conf.staffs.some(x => message.member.roles.cache.has(x))) {
    const num = nums.get(message.author.id);
    if (num && (num % god.Main.messageCount) === 0) {
      nums.set(message.author.id, num + 1);
      await coin.findOneAndUpdate({ guildID: message.guild.id, userID: message.author.id }, { $inc: { coin: god.Main.messageCoin } }, { upsert: true });
  ...
  await dolar.findOneAndUpdate({ guildID: message.guild.id, userID: message.author.id }, { $inc: { dolar: god.Main.messageDolar } }, { upsert: true });
  }
