...
module.exports = {
  ...
  async execute(interaction, bot) {
    if(!god.owners.includes(interaction.user.id)) {
      return interaction.reply({ content: "<a:warnn:1117866163565772854> **Bot developerı olmadığın için kurulumu yapamazsınız.**", ephemeral: true })
    }
...
if(RoleModel) {await RoleModel.deleteMany({});}
  ...
  const guild = bot.guilds.cache.get(god.GuildID);
