...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    if(!member) {
        return interaction.reply({ content: `**Belirttiğiniz üye bulunmamaktadır.**`, ephemeral: true })
        ...
        return interaction.reply({ content: "Yetkili Alım Rolüne Sahip olmadığın için cevaplayamazsın.", ephemeral: true })
      } else if([conf.staffs].some(role2 => member.roles.cache.get(role2))) {
        return interaction.reply({ content: "Belirttiğin kullanıcı Zaten Yetkili Rolüne Sahip olduğu için başvurusunu cevaplayamazsın.", ephemeral: true })
      } else  {
